# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""

REDEEM_SCRIPT=$(bitcoin-cli -regtest decoderawtransaction "$transaction" \
  | jq -r '.vin[0].scriptSig.asm' \
  | awk '{print $NF}')
 
python3 << PYEOF
import hashlib
 
redeem_script = bytes.fromhex("$REDEEM_SCRIPT")
 
# Step 1: SHA256(redeemScript) = 32-byte witness program
witness_program = hashlib.sha256(redeem_script).digest()
 
# Step 2: P2WSH script = OP_0 + OP_PUSHDATA32 + witness_program
p2wsh_script = bytes([0x00, 0x20]) + witness_program
 
# Step 3: HASH160(p2wsh_script) = SHA256 then RIPEMD160
sha = hashlib.sha256(p2wsh_script).digest()
rmd = hashlib.new('ripemd160')
rmd.update(sha)
script_hash = rmd.digest()
 
# Step 4: Base58Check encode with testnet/regtest P2SH prefix 0xC4
def b58check(prefix, payload):
    data = bytes([prefix]) + payload
    cksum = hashlib.sha256(hashlib.sha256(data).digest()).digest()[:4]
    full = data + cksum
    alpha = '123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz'
    n = int.from_bytes(full, 'big')
    out = ''
    while n > 0:
        n, r = divmod(n, 58)
        out = alpha[r] + out
    return out
 
print(b58check(0xC4, script_hash))
PYEOF